    ...
    <body>
        <div class="x-nav">
            <span class="layui-breadcrumb">
              ...
              <a><cite>Management Zone Indices</cite></a>
            </span>
        </div>
        <div class="x-body">
          <div tabindex="300" class="btn btn-info btn-lg btn-file" >
            ...
            <table class="layui-table">
                <thead>
                    <tr>
                        <th>zone_id</th>
                        <th>belong_river_id</th>
                        <th>belong_catchment_id</th>
                        <th>management_zone_name</th>
                ...
                <?php
                    ...
                    $result=mysqli_query($conn,"SELECT * FROM management_zone_indices");  
                    $dataCount=mysqli_num_rows($result);    
                    for($i=0;$i<$dataCount;$i++){  
                        $result_arr=mysqli_fetch_assoc($result); 
                        $zone_id=$result_arr['zone_id'];  
                        $belong_river_id=$result_arr['belong_river_id'];  
                        $belong_catchment_id=$result_arr['belong_catchment_id']; 
                        $management_zone_name=$result_arr['management_zone_name'];
                        ...
                        echo "<tr><td>$zone_id</td><td>$belong_river_id</td><td>$belong_catchment_id</td>"
                            . "<td>$management_zone_name</td><td>$lt_annual_extraction_limit</td><td>$license_category_1</td>"
                            . "<td>$license_category_2</th><tr>";
                        }
                ?>
                </tbody>
            </table>
       ...
       <script>
            function delete_table(){
                ...
                if (r==true){
                    ...
                    xhttp.open("POST", "../../tools/db_table_delete.php?table_name=management_zone_indices", true);
                    xhttp.send();
                }
            }
                ...
                req.onreadystatechange = function() {
                    if(req.readyState === 4 && req.status === 200) {
                        if(this.responseText=="1"){
                            window.open("../../files/export.files/management_zone_indices.csv");
                        }else{
