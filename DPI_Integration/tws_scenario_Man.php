...
<html>
    <head>
        <title>TWS Insight for Manning</title>
	...
	<div id="page-wrapper" class="gray-bg dashboard"  style="padding-bottom:20px">
        ...
        //purpose_des, share_component, longitude, latitude
            include 'db.helper/db_connection_ini.php';
                if($conn!=null){
                    $sql = "SELECT * FROM town_water_supply WHERE catchment='Manning'";
        ...
        <script type="text/javascript">
            ...
            var lgas = lga_man;
            var lgaCentroids = lgaCentroids_man;
            var lga = new Array();
            var padding = 35;
            var layer, overlay;
            var filtered;
            ...
            document.getElementById('s0_title').innerHTML = '<span style="font-size:18px; font-weight:bold; margin-bottom: 0; height: 48px;">'+'Town Water Supply Insight for Manning Catchment'+'</span>';
		...
		L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=' + myToken, {
            ...
            CAT = L.geoJSON(ManningRiver_CatchmentBoundary, {
            style: function (feature) {
                    return { color: 'grey', weight: 0.3, fillOpacity: 0.1};
            }
            }).addTo(map);
            
            var Man_lga = L.geoJSON(lgas, {
...
//            hover_info.addTo(map);
            ...
            function show_s1(id){
                    ...
                    d3.csv("data/town_water_supply_wsdi_man.csv", function (data) {
...
//                                    legend_1.addTo(map);
            ...
            function show_s2(id){
                    ...
                    var max_row_pa=0;//Get the row number of ranking file
                    d3.csv("data/town_water_supply_wsr_man.csv", function (data) {
                        _.each(data, function (d, i) {
                        max_row_pa++;
                        });
                    });
...
//                                    legend_1.addTo(map);
            ...
            function show_s3(id){
                var checkBox = document.getElementById(id); 
                if (checkBox.checked === true){
                    document.getElementById('s0_title').innerHTML = '<span style="font-size:18px; font-weight:bold; margin-bottom: 0; height: 48px;">'+'Town Water Supply Insight for Manning Catchment--Health risk due to poor water quality'+'</span>';
                    var elem_ov = document.createElement("div");
                    elem_ov.setAttribute('id', 'tws_legend');
                    elem_ov.innerHTML = (
